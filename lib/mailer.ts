...
export function buildReminderEmail({
  ...
  const messages = {
    before_due: {
      subject: `⏰ Upcoming: "${taskTitle}" is due soon`,
    ...
    overdue: {
      subject: `🚨 Overdue: "${taskTitle}" — action required`,
    ...
    <div class="card">
      ...
      <div class="task-block">
        ...
        <div class="meta">
          ...
          Priority: <strong>${priorityLabel} (${priority}/10)</strong>
          ${label ? ` &nbsp;|&nbsp; Label: <strong>${label}</strong>` : ""}
        </div>
      </div>
      <a href="${process.env.NEXT_PUBLIC_APP_URL ?? ""}/tasks" class="cta">View Tasks →</a>
      <div class="footer">Sent by TaskChaser. Visit the app to manage your tasks.</div>
    </div></body></html>`;
